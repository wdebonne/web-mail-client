...
Limites : 64 KiB par valeur (porté à **4 MiB** pour les clés préfixées `mail.signatures.` et `mail.templates.` afin d'autoriser les signatures riches contenant des images base64), 500 entrées max par requête PUT, clés filtrées par `/^[a-zA-Z0-9_.\-:]{1,255}$/`.
