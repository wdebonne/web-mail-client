...
- Nouveau bouton **Conversation** dans le groupe *Disposition* de l'onglet **Afficher** (modes classique et simplifié). Lorsqu'il est activé, la liste regroupe les messages par **fil de discussion** au lieu de les regrouper par date ; chaque groupe affiche le sujet du thread et la liste est ordonnée selon la date du dernier message du fil. Le regroupement utilise les en-têtes RFC 822 (`References`, `In-Reply-To`, `Message-ID`) avec, en dernier recours, le sujet normalisé (préfixes `Re:`/`Fwd:`/`Tr:`/`Rép:` retirés). **Désactivé par défaut** ; préférence persistée (`conversationView`).
