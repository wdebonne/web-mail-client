...
- Nouveau bouton **Conversation** dans le groupe *Disposition* de l'onglet **Afficher** (modes classique et simplifié). Lorsqu'il est activé, la **vue du mail** affiche un bandeau « Conversation » listant tous les messages du fil (reçus + répondus), triés par date ; un clic sur une entrée ouvre le message correspondant dans la même vue. Le regroupement utilise les en-têtes RFC 822 (`References`, `In-Reply-To`, `Message-ID`) avec, en dernier recours, le sujet normalisé (préfixes `Re:`/`Fwd:`/`Tr:`/`Rép:` retirés). **La liste des mails n'est pas regroupée** (tri par date conservé) ; un petit icône de conversation apparaît simplement sur les lignes appartenant à un fil contenant plusieurs messages. **Désactivé par défaut** ; préférence persistée (`conversationView`).
