...
- **Cache local complet et synchronisation incrémentale**
  - Le cache ne se rafraîchissait jamais : chaque « mise à jour » redemandait la **page 1 de tous les dossiers de tous les comptes**, rouvrait une connexion IMAP par appel et réécrivait `cached_emails`, pour comparer une empreinte *après* avoir tout retéléchargé. Il n'existait aucune notion d'**UIDVALIDITY, UIDNEXT ou MODSEQ** dans le projet : rien ne permettait de savoir ce qui avait changé, donc tout était refait.
  - Il n'était pas complet non plus : **50 messages par dossier et aucun corps**, malgré un commentaire promettant le contraire depuis l'origine.
  - **Le poste détient désormais tout** — en-têtes *et* corps de tous les messages de tous les dossiers. L'affichage d'un dossier et la recherche se lisent dans IndexedDB, sans aller-retour réseau. Les pièces jointes restent en métadonnées seules ; leurs octets ne sont récupérés qu'à l'ouverture. C'est un choix assumé : elles représentent l'essentiel du poids et presque rien de la valeur d'usage.
  - **La synchronisation est devenue un delta.** Un cycle commence par un `STATUS` par compte, sur une seule connexion IMAP. UIDNEXT ne faisant que croître, l'y voir inchangé prouve qu'aucun message n'a été ajouté ; combiné à un nombre de messages inchangé, cela exclut aussi toute suppression. Le raccourci est **exact, pas heuristique** — et dans le cas courant le cycle s'arrête là, sans rien télécharger. Là où l'application émettait un `FETCH` de 50 enveloppes toutes les 30 secondes par dossier ouvert, elle émet un `STATUS` par compte toutes les 60 secondes.
  - **Un changement d'UIDVALIDITY ne purge que le dossier concerné.** Une renumérotation sur un dossier ne dit rien des autres ; les vider tous imposerait une resynchronisation complète sans raison.
  - **La première synchronisation est longue, visible et reprenable.** Les en-têtes descendent par lots de 500, les corps par lots de 25, en rendant la main au navigateur entre chaque lot. La progression est persistée après *chaque* lot : fermer l'onglet en plein remplissage ne coûte que le lot en cours. Deux barres distinctes dans l'indicateur et dans les réglages — confondre une tâche de quelques secondes avec une tâche de plusieurs minutes donnerait une jauge illisible — et un bouton **Suspendre / Reprendre**.
  - **Mettre à jour l'application ne coûte plus rien.** Le schéma IndexedDB passe en v3 en *ajoutant* les nouveaux magasins et index, sans jamais reconstruire l'existant ; une passe de rattrapage complète en tâche de fond les enregistrements hérités, en relisant sur place les corps déjà présents. Un futur changement de tokenisation déclenchera une **ré-indexation locale**, pas un retéléchargement.
  - **La recherche trouve enfin dans le contenu des messages.** Index inversé natif d'IndexedDB (`terms` en `multiEntry`), avec correspondance par préfixe — « factu » trouve « facture » — et normalisation sans accents, indispensable en français où presque personne ne les tape. Les trois portées (dossier courant, cette boîte, partout) sont servies localement et instantanément. Le repli sur `/api/search` subsiste tant qu'un dossier de la portée n'est pas entièrement rapatrié : annoncer un résultat local comme exhaustif alors qu'il ne l'est pas serait pire que d'être lent.
  - **Les boîtes de réception se remplissent en premier.** L'ordre naturel des clés IndexedDB étant alphabétique, « Archive » passait avant « INBOX » : le seul dossier que l'utilisateur regarde en attendant finissait de se remplir en dernier. Le remplissage suit désormais l'usage réel — réception, envoyés, brouillons, sous-dossiers, puis archive et corbeille — et, à priorité égale, traite le plus petit dossier d'abord pour que la progression soit perceptible.
  - **Les images incorporées aux messages s'affichent enfin.** Un message à signature ou en-tête illustré référence ses images par `<img src="cid:…">` ; cette syntaxe ne veut rien dire pour un navigateur, et **rien ne la résolvait** : toutes ces images étaient des liens morts. Elles sont maintenant rapatriées avec le corps (plafonnées à 256 Ko par image et 1 Mo par message) et conservées en base64, donc affichées sans aucune conversion. Une image sans correspondance voit son `src` retiré plutôt que laissé tel quel — un espace vide vaut mieux que l'icône de lien mort.
  - Ces images font délibérément exception à la règle « aucun octet de pièce jointe » : ce sont des logos et des signatures de quelques dizaines de Ko, elles font partie du **rendu** du message et non de ses annexes. Un message mis en cache s'affiche donc complet, hors-ligne compris.
  - **Le cache d'images distantes passe de 1 000 entrées / 7 jours à 20 000 / 6 mois.** À l'ancien réglage, revenir sur un message d'il y a dix jours retéléchargeait toutes ses images, et une seule lettre d'information illustrée suffisait à évincer le reste. Le poste a de la place ; le temps d'affichage, lui, se voit.

  - **L'ouverture d'un message affiche son contenu immédiatement.** Le corps en cache est peint sans attendre le réseau, puis remplacé par la version du serveur dès qu'elle arrive — seule celle-ci porte les images inline et le contenu des pièces jointes. La requête réseau part **avant** la lecture du cache pour ne rien retarder, et la peinture est abandonnée si le réseau a déjà répondu : repeindre par-dessus avec la version allégée serait un retour en arrière visible.
  - **Échec de suppression : le cache local est rétabli.** La liste visible était bien restaurée, mais les messages avaient déjà été retirés d'IndexedDB — et le delta suivant ne les aurait pas ramenés, la suppression ayant échoué côté serveur, le dossier apparaissait structurellement inchangé. Un relevé forcé est déclenché sur ce chemin d'erreur.
  - **Ce que la recherche couvre désormais** : l'**objet**, l'**expéditeur** (nom et adresse), les **destinataires — y compris ceux en copie** (nom et adresse), le **corps** du message, et le **nom des fichiers joints** (« devis » retrouve `Devis-2026-Toiture.pdf`). Les noms de pièces jointes et les correspondants sont indexés **en tête** : courts et très discriminants, ils survivent à la troncature là où un corps volumineux serait écrêté. Le filtre *Avec / Sans pièce jointe* reste par ailleurs disponible dans les pastilles. Les destinataires en copie cachée ne sont pas indexés : ils n'apparaissent pas dans l'enveloppe IMAP d'un message reçu.
  - **Le contenu des documents joints est indexé** (`.pdf`, `.docx`, `.xlsx`, `.xls`, `.csv`, `.txt`) : chercher « toiture » retrouve le message dont le devis joint mentionne une réfection de toiture. L'extraction a lieu **à l'ouverture du message**, et nulle part ailleurs — `getMessage` vient d'en télécharger les octets pour l'afficher, donc l'indexation ne coûte **aucun octet de réseau supplémentaire**. L'extraire pendant le remplissage aurait imposé de rapatrier toutes les pièces jointes de la boîte, plusieurs giga-octets, et aurait détruit exactement ce qui rend un cache complet abordable. Les bibliothèques employées (`mammoth`, `xlsx`) étaient déjà présentes pour l'aperçu des pièces jointes et restent en import dynamique : aucune dépendance ajoutée, aucun poids dans le bundle principal.
  - **Le PDF est traité pour sa couche texte.** Les multifonctions récentes océrisent à la numérisation, si bien qu'un PDF scanné en possède une dans la grande majorité des cas ; celui qui n'en a pas ressort simplement vide, sans erreur — le message reste trouvable par son objet, ses correspondants, son corps et le nom du fichier. Aucun OCR n'est fait ici : il coûterait une dizaine de Mo de bibliothèque et plusieurs secondes par page.
  - **PDF.js est ajouté en 6.2.108, la première version corrigée de l'avis GHSA-hq66-cqwq-w95j** — exécution de JavaScript arbitraire à l'ouverture d'un PDF malveillant, sévérité haute, qui touche toutes les versions `>= 5.6.83 < 6.2.108`. Dans un client mail, les pièces jointes viennent d'expéditeurs quelconques : c'est précisément le scénario visé par cet avis. L'extraction est en outre configurée pour ne rien faire d'autre que lire du texte — `isEvalSupported: false`, aucun moteur de script, aucune police résolue, aucun rendu.
  - Le chunk PDF.js (~535 Ko) et son worker (~1,3 Mo) sont **chargés à la demande et exclus du précache** du service worker. Les précacher aurait imposé ~530 Ko à l'installation à tous les utilisateurs, y compris ceux qui ne reçoivent jamais de PDF, pour une disponibilité hors-ligne sans objet : l'extraction n'a lieu qu'à l'ouverture d'un message, donc en ligne. Le poids du bundle principal est inchangé (+5 Ko).
  - Le `.pptx` reste non couvert : il demanderait `jszip`, présent seulement en dépendance transitive.
  - Le plafond de termes passe de 200 à 300, dont **100 réservés au contenu des documents joints** — le reliquat inutilisé revenant au corps, et réciproquement. Sans cette réserve, un corps un peu bavard consommait tout le budget et le document n'entrait jamais dans l'index : la fonction aurait paru marcher sur les messages courts et échouer sur les autres, sans logique apparente.
  - Quatre nouvelles routes serveur **sans état** (`POST /api/sync/accounts/:id/{state,envelopes,bodies}`, `GET .../uidflags`). Le serveur n'acquiert **aucune table** : c'est le poste, qui connaît l'état précédent, qui calcule le delta. Les drapeaux voyagent en masque binaire — pour 10 000 messages, ~120 Ko au lieu de ~900 Ko, ce qui rend supportable la vérification d'un dossier entier sans dépendre de CONDSTORE, que beaucoup de serveurs n'annoncent pas.
  - La récupération des corps en masse **ne télécharge jamais les pièces jointes** : elle lit la `BODYSTRUCTURE` et ne rapatrie que les parties texte. Sur un message de 8 Mo dont le corps fait 12 Ko, c'est 12 Ko. `getMessage` reste inchangé pour l'ouverture d'un message : le rendu fidèle, images inline comprises, continue de passer par lui. Une extraction imparfaite sur un message exotique dégrade donc la recherche, jamais l'affichage.
  - Le cache HTTP `/api/mail/**` du service worker est **supprimé**. Il n'avait ni plafond d'entrées ni durée de vie, grossissait sans limite, et « Purger le cache » ne le vidait même pas ; il fait désormais double emploi avec IndexedDB, et servir une liste périmée depuis là contredirait le protocole de delta. L'espace déjà accumulé est récupéré au premier démarrage du nouveau service worker.
  - Le panneau **Réglages → Cache local** compte désormais par index plutôt qu'en chargeant tous les enregistrements en mémoire — avec un cache complet, l'ancienne méthode aurait fait tomber l'onglet. Il indique la couverture des corps dossier par dossier et si le stockage est marqué **persistant** ; sans cette marque, le navigateur peut évincer l'origine sous pression disque et réduire à néant des heures de remplissage.
  - Ordre de grandeur à connaître avant de se lancer : compter **30 à 60 Ko par message** avec son corps, plus environ 1,6 Ko d'index. Une boîte de 50 000 messages occupe donc quelques giga-octets. Au-delà de 85 % du quota, les **corps** les plus anciens sont évincés — jamais les en-têtes, qui sont légers et sont ce qui fait vivre l'affichage, alors qu'un corps se retélécharge à l'ouverture.

- **Assistant IA (Ollama) — configurable, et enfin utilisable**
  - Nouvel écran **Administration → Assistant IA (Ollama)** : URL du serveur, modèle, clé d'API facultative (chiffrée), langue des réponses, température, longueur maximale, délai d'attente, troncature du texte envoyé, et activation séparée de chaque fonction.
  - Le bouton **Lister les modèles** interroge le serveur Ollama (`/api/tags`) et remplit la liste déroulante ; le champ texte reste ouvert pour un modèle pas encore téléchargé.
  - Le **diagnostic** teste dans l'ordre où les choses cassent en pratique : joignabilité, modèles présents, modèle sélectionné, puis **une génération réelle** — seul test qui prouve que le modèle tient dans la RAM de la machine et répond dans le délai imparti. Il s'arrête à la première étape en échec, les suivantes n'ayant plus rien de fiable à dire. Il s'exécute **depuis le serveur applicatif**, pas depuis le navigateur de l'administrateur : c'est le seul point de vue qui compte, et l'écart entre les deux est le piège numéro un en conteneur (`localhost` y désigne le conteneur, pas l'hôte). Le message d'erreur le rappelle explicitement.
  - Modèle *et* configuration se testent **avant enregistrement** : sans ça, il fallait sauvegarder une URL fausse pour découvrir qu'elle était fausse.
  - Trois fonctions côté utilisateur : **Résumer** un message (volet de lecture), **Proposer une réponse** et **Réécrire mon texte** (fenêtre de rédaction, barre *Assistant IA* au-dessus de l'éditeur, avec ton et consigne libre).
  - **Le texte généré n'écrase jamais un brouillon.** Il s'affiche d'abord dans un aperçu, et c'est l'utilisateur qui décide de l'insérer, de le copier ou de l'écarter. Un modèle local part parfois complètement à côté ; perdre un brouillon pour ça serait impardonnable. Le texte inséré est échappé avant d'entrer dans l'éditeur `contentEditable`.
  - **La réécriture ignore la citation** du message d'origine : faire réécrire le mail auquel on répond en même temps que sa propre réponse ne produit rien d'exploitable.
  - **Rien ne sort de l'infrastructure** : les appels partent du serveur applicatif, jamais du navigateur, qui ne connaît ni l'URL ni la clé du serveur de modèles. Nouvelles routes `GET /api/ai/status`, `POST /api/ai/{summarize,reply,improve}`, plafonnées à **30 requêtes par 5 minutes et par compte** — une génération occupe le GPU du serveur Ollama pendant plusieurs secondes, et un seul utilisateur qui pilonne un bouton mettait la file à genoux pour tout le monde. Le décompte est par utilisateur et non par IP : derrière un proxy d'entreprise, tout le monde partage la même adresse.
  - Le bloc `<think>` des modèles de raisonnement (deepseek-r1, qwen3…) est retiré des réponses — sans quoi le brouillon du modèle se serait retrouvé dans le mail.
  - Le plugin `ollama-ai` du dossier `plugins/` **n'a jamais fonctionné** : jamais inséré en base, aucun écran d'installation ni de configuration côté administration, et une interface (`init` / `executeAction`) qui ne correspondait pas à ce que le `PluginManager` appelle (`initialize` / `module[action]`). Il est laissé en place mais n'est plus référencé ; la documentation qui renvoyait vers *Administration → Plugins* est corrigée.
...
- **L'adresse IP du client n'est plus falsifiable par en-tête** — six emplacements retenaient le *premier* élément de `X-Forwarded-For`, alors que le nginx documenté utilise `$proxy_add_x_forwarded_for`, qui **ajoute** l'IP réelle à la valeur envoyée par le client au lieu de la remplacer. Un simple `curl -H 'X-Forwarded-For: 192.168.1.5'` suffisait donc à choisir son adresse, et donc à contourner la **liste noire d'IP** sur tous les chemins de connexion ainsi que le **filtre réseau Kerberos**. Nouveau helper `server/src/utils/clientIp.ts` renvoyant `req.ip` : `trust proxy` étant déjà positionné, Express résout l'en-tête sur le dernier saut — celui ajouté par le reverse proxy, le seul qu'un client ne puisse pas écrire. Le limiteur de débit n'était pas concerné, `express-rate-limit` s'appuyant déjà sur `req.ip`. Les adresses enregistrées dans `login_attempts`, le journal d'audit et l'alerte « nouvel appareil » sont fiables à leur tour.
- **Kerberos — un compte désactivé dans l'AD conservait l'accès.** Le filtre de résolution par défaut écarte désormais les comptes désactivés (`(!(userAccountControl:1.2.840.113556.1.4.803:=2))`). Ce chemin ne fait aucun `bind` LDAP, contrairement à la connexion par mot de passe : sans ce terme, rien ne voyait la désactivation, et le poste gardait l'accès jusqu'à l'expiration du ticket en cache (10 h par défaut) — la session d'appareil émise dans l'intervalle valant ensuite 90 jours. Ce n'est qu'un **défaut** : les installations ayant personnalisé le filtre gardent le leur, et la variante pour annuaire sans `userAccountControl` (OpenLDAP) est documentée.

### Corrigé

- **La recherche serveur ignorait les boîtes partagées.** `/api/search` ne filtrait que sur `mail_accounts.user_id`, alors que `getAccountForUser` honore aussi les attributions de boîte et les partages : un utilisateur ne trouvait rien dans une boîte qu'il pouvait pourtant ouvrir et parcourir. Le prédicat d'accès est extrait dans `server/src/utils/accountAccess.ts` et partagé par les deux chemins, pour qu'ils ne puissent plus diverger.
- **La recherche serveur faisait un parcours séquentiel complet à chaque requête.** L'index GIN `to_tsvector` créé sur `cached_emails` était **inutilisable** par la requête, qui interroge la table en `ILIKE '%…%'` — deux mécanismes incompatibles. Il est remplacé par des index trigrammes (`pg_trgm`), que le planificateur sait effectivement utiliser pour un `ILIKE`, sans réécrire la requête. `pg_trgm` étant une extension « trusted » depuis PostgreSQL 13, le propriétaire de la base peut la créer sans privilège particulier ; si l'hébergeur la refuse, l'application le signale et continue de fonctionner comme avant.
- **La clause `body_text ILIKE` de la recherche ne pouvait rien trouver** : la colonne n'est jamais renseignée par `cacheMessages`, et sa présence empêchait par ailleurs l'usage des index trigrammes. Elle est retirée — la recherche dans le contenu est désormais assurée localement, sur le cache complet du poste.
- **Un même UID existant dans deux dossiers pouvait renvoyer le mauvais message.** Le repli en cache de `GET /api/mail/accounts/:id/messages/:uid` interrogeait `cached_emails` sans filtre de dossier, alors que le dossier fait partie de l'identité d'un message.
- **Les messages déplacés par le filtre indésirable ou par une règle laissaient une ligne fantôme en cache**, qui continuait indéfiniment de remonter dans les résultats de recherche alors que le message n'était plus là. Nouveau helper `server/src/utils/emailCache.ts`, appelé depuis tous les points de sortie.

- **Kerberos — les réseaux autorisés acceptent enfin IPv6.** `ipMatchesCidr` ne savait comparer que des adresses IPv4 et l'enregistrement rejetait en 400 tout CIDR IPv6. Or Windows privilégie IPv6 dès qu'il est disponible : un administrateur qui renseignait `192.168.10.0/24` désactivait la connexion intégrée pour tous les postes joignant le serveur en IPv6 — **sans aucun message**, la page de connexion étant conçue pour échouer en silence. La comparaison se fait maintenant sur les octets de l'adresse, donc par préfixe de bits, pour les deux familles (un `/64` IPv6 ne tient pas dans un entier 32 bits) ; les familles ne se croisent jamais. Le serveur écrit en plus un avertissement dans ses journaux quand il écarte un client IPv6 alors que tous les réseaux déclarés sont en IPv4, ce qui était le mode d'échec le plus coûteux à diagnostiquer.
- **Application de bureau — la connexion Windows intégrée fonctionne désormais dans le client lourd.** WebView2 est un processus Chromium distinct du navigateur Edge : il n'hérite ni de la zone *Intranet local* ni de la stratégie `AuthServerAllowlist`, et ignorait donc le défi `Negotiate` — l'application affichait le formulaire alors que le même serveur se connectait en zéro clic dans le navigateur du poste. L'application pose maintenant l'argument nécessaire au démarrage, à partir de l'URL du serveur avec laquelle elle a été compilée. Seul **l'hôte exact** est autorisé, jamais le domaine entier, et la délégation Kerberos n'est pas activée : le serveur valide le ticket, il n'a jamais à agir au nom de l'utilisateur auprès d'un tiers. `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS` défini dans l'environnement reprend la main, même convention que `KRB5_KTNAME` / `KRB5_CONFIG` côté serveur.
...
- **Gestion du courrier indésirable**
  ...
  - **« Ce n'est pas indésirable »** — le geste symétrique, disponible en bandeau permanent dès qu'on lit un message rangé dans le dossier indésirable, et dans le clic droit : débloque l'expéditeur (adresse *et* domaine), l'ajoute aux autorisés, et remet le message en boîte de réception. La réversibilité en un clic est ce qui rend la fonction utilisable sans crainte.
  - **Filtre serveur** : exploitation des en-têtes posés en amont par SpamAssassin / Rspamd / Exchange (`X-Spam-Flag`, `X-Spam-Status`, `X-Spam-Score`, `X-Spam-Level`), réglable en trois niveaux — *Désactivé* (seules les listes s'appliquent), *Normal* (score ≥ 5), *Strict* (score ≥ 3). Aucun apprentissage ni modèle local : on lit un verdict déjà calculé. Le drapeau `X-Spam-Flag: YES` déclenche le classement quel que soit le niveau, le réglage ne servant qu'à **abaisser** le seuil de score.
  - **La liste des expéditeurs autorisés prime toujours** sur tout le reste, y compris le verdict du serveur — avec l'option **« Toujours faire confiance à mes contacts »** (activée par défaut) qui étend cette garantie au carnet d'adresses. Ajouter une valeur dans une liste la retire automatiquement de l'autre.
  - **Désabonnement en un clic** : quand un message annonce un mécanisme `List-Unsubscribe`, un bandeau propose **Se désabonner**. Le serveur privilégie le POST *one-click* (RFC 8058) quand il est annoncé, sinon envoie l'e-mail `mailto:` depuis le compte concerné, sinon rend l'URL au client qui l'ouvre dans un onglet.
  - Nouvel onglet **Paramètres → Courrier indésirable** : interrupteur général, niveau de protection, confiance aux contacts, **vidage automatique** du dossier (jamais / 7 / 30 / 90 jours), bouton **« Nettoyer maintenant »** (applique les réglages aux 100 derniers messages), et gestion des deux listes.
  - Nouvelle page **Admin → Messagerie → Courrier indésirable** : activation globale de la fonctionnalité, valeurs par défaut (appliquées aux seuls utilisateurs n'ayant jamais touché à leurs réglages, sans écraser les choix déjà faits), et **listes globales** bloqués / autorisés visibles en lecture seule côté utilisateur.
  - API : `/api/junk/{settings,senders,block,not-junk,unsubscribe,sweep,explain}` et `/api/admin/junk/{settings,senders}`. Nouvelles tables `junk_senders`, `junk_settings`, `junk_scan_state`.
  - Le service de fond `junkFilter` (cycle 2 min) est **volontairement indépendant du poller de nouveaux mails** : celui-ci ne traite que les comptes ayant une souscription push active ou un répondeur automatique, et plafonne à 5 messages par cycle — un filtre greffé dessus aurait été silencieusement inopérant pour la majorité des boîtes. Le poller appelle malgré tout le filtre en ligne, uniquement pour qu'un message classé indésirable ne déclenche ni notification ni réponse automatique (répondre à un spam confirme l'adresse à l'expéditeur). Il apparaît dans **Admin → État du système** et est couvert par les alertes système.
  - **Le service de fond ne touche jamais aux messages déjà reçus** (curseur `junk_scan_state` par compte) : les seuls déplacements rétroactifs viennent de la case du dialogue de blocage et du bouton « Nettoyer maintenant », donc toujours d'une action explicite. Au premier passage sur un compte, seul le repère est posé.
  - Limites connues : au-delà de **50 nouveaux messages entre deux cycles**, les plus anciens sont sautés (le curseur avance quand même) — rattrapables via « Nettoyer maintenant ». Si le serveur de messagerie n'analyse pas le spam, seules les listes d'expéditeurs s'appliquent.

- **Bloc-notes intégré + lecture des fichiers Nextcloud dans la composition**
  - Nouveau **panneau latéral « Notes & fichiers »**, ouvert depuis le ruban **Insérer** de la fenêtre de composition — même mécanique que le panneau Emojis (largeur, chrome, insertion au point du curseur mémorisé avant l'ouverture du panneau).
  - Onglet **Notes** : recherche plein texte, création rapide (`Ctrl+Entrée`), insertion en un clic dans le corps du message, copie dans le presse-papier, épinglage.
  - Onglet **Fichiers** (visible uniquement si le compte Nextcloud est lié) : navigation et recherche dans le drive de l'utilisateur, puis **extraction du contenu** du fichier pour le coller dans le message — texte / Markdown / CSV / JSON mis en paragraphes, **.docx** converti via *mammoth*, **.xlsx / .ods** rendus en tableau HTML bordé, images insérées en data URI. Actions complémentaires : *Copier le texte*, *Joindre le fichier*, *Enregistrer comme note* (le chemin d'origine est conservé sur la note).
  - **Grande modale** (`Notes & fichiers`) accessible depuis un **bouton bloc-notes dans la barre du haut, à gauche de l'indicateur de cache** ou via *Ouvrir en grand* depuis le panneau : colonne de gauche = source (mes notes / Nextcloud) + recherche, colonne de droite = éditeur de note (titre, gras/italique/souligné, listes, lien, couleur, épingle, suppression, `Ctrl+S`) ou aperçu du fichier. Elle est **montée une seule fois dans le `Layout`**, donc joignable depuis Messagerie, Calendrier, Contacts, etc.
  - Hors composition, les actions d'insertion deviennent **« Nouveau message avec cette note / ce contenu »** : la composition est pré-remplie via le `mailStore` puis l'utilisateur est renvoyé sur `/mail`.
  - Le raccourci de la barre du haut est optionnel — **Paramètres → Apparence → Barre du haut** (clé `ui.notesShortcut`, activée par défaut, synchronisée entre appareils et incluse dans la sauvegarde). Désactivé, tout reste accessible depuis le ruban **Insérer**.
  - API : `GET/POST /api/notes`, `GET/PUT/DELETE /api/notes/:id`. Nouvelle table `notes` (par utilisateur, sans partage) avec index GIN plein texte français ; le HTML est assaini côté serveur avec la même liste blanche que les modèles de mail, et une projection texte (`content_text`) est recalculée à chaque écriture pour la recherche et les extraits. Côté Nextcloud, les routes `/api/nextcloud/files/*` existantes sont réutilisées telles quelles.
  - Limites connues : **le texte des PDF n'est pas extrait** (aucune bibliothèque PDF dans les dépendances) — le fichier s'affiche dans la visionneuse native du navigateur, où le texte reste sélectionnable et copiable, et il reste joignable au message. L'extraction est plafonnée à **8 Mo** (le fichier transite en base64 et est traité en mémoire dans l'onglet). La recherche *dans le contenu* des fichiers dépend du fournisseur de recherche unifiée de Nextcloud : sans l'application *Full text search*, la recherche porte sur les noms de fichiers.
...
- **Alerte « connexion depuis un appareil inconnu »**
  ...
  - **Identité d'appareil fiabilisée** : un cookie httpOnly longue durée (`wm_device`, 1 an, prolongé à chaque connexion et à chaque refresh, jamais effacé au logout) est posé au premier login et identifie le navigateur indépendamment du User-Agent — un appareil reste reconnu après une mise à jour de navigateur, et un attaquant utilisant le même couple « Chrome · Windows » que la victime déclenche bien l'alerte. Sans cookie (migration depuis une version antérieure, cookies effacés), repli sur **nom d'appareil + même sous-réseau IP** (/24 en IPv4, /64 en IPv6) — le nom seul ne suffit plus. Table `known_devices` (hash SHA-256 du token, une ligne par couple utilisateur/navigateur : sur un poste partagé, chaque compte enregistre le même token, sans fausses alertes en alternance).
...
- **Filtre des groupes LDAP synchronisés** (report S3 de l'audit) : nouveau réglage **Admin → LDAP → Synchronisation des groupes** — préfixe de nom (CN), expression régulière sur le CN, ou OU de base — appliqué avant la création automatique des groupes à la connexion, pour écarter les groupes techniques d'un annuaire d'entreprise. Un groupe exclu n'est plus créé ni synchronisé (l'utilisateur en est retiré à sa prochaine connexion) ; les mappings manuels et la détection des groupes admin ne sont pas concernés. Une regex invalide est rejetée à l'enregistrement et ne synchronise aucun groupe.
