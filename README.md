...
- 🎯 **Couverture** : objet, expéditeur, **destinataires y compris en copie**, **corps du message**, **nom des fichiers joints** (« devis » retrouve `Devis-2026-Toiture.pdf`) et **contenu des documents bureautiques joints** — `.docx`, `.xlsx`, `.xls`, `.csv`, `.txt`. Le contenu d'un document joint est extrait à l'ouverture du message, quand ses octets viennent d'être téléchargés pour l'affichage : l'indexation ne coûte alors aucun transfert supplémentaire.
- 🚫 **Non couvert** : le contenu des PDF (un PDF scanné n'a pas de couche texte, et une couverture aléatoire se lirait comme une panne), le `.pptx`, et les destinataires en copie cachée (absents de l'enveloppe IMAP).
