...
- 🎯 **Couverture** : objet, expéditeur, **destinataires y compris en copie**, **corps du message**, **nom des fichiers joints** (« devis » retrouve `Devis-2026-Toiture.pdf`) et **contenu des documents joints** — `.pdf` (couche texte), `.docx`, `.xlsx`, `.xls`, `.csv`, `.txt`. Le contenu d'un document joint est extrait à l'ouverture du message, quand ses octets viennent d'être téléchargés pour l'affichage : l'indexation ne coûte alors aucun transfert supplémentaire.
- 🚫 **Non couvert** : les PDF sans couche texte (numérisation non océrisée — rare, les multifonctions récentes océrisant par défaut), le `.pptx`, et les destinataires en copie cachée (absents de l'enveloppe IMAP). Aucun OCR n'est embarqué.
...
- 🔔 Rappels
- 🖨️ **Impression du calendrier** : le bouton *Imprimer* du ruban ouvre une modal dédiée — sélection des agendas à imprimer, choix de la vue (Jour / Semaine / Semaine de travail / Mois), intervalle de temps, prévisualisation live, puis génération d'un document A4 paysage optimisé pour l'impression.
...
- 🔒 **Sécurité — protection des connexions** : verrouillage automatique après N tentatives échouées (seuil configurable, défaut 3), durée de verrou réglable (0 = permanent). **Liste noire** d'IPs bloquées immédiatement. **Liste blanche** d'IPs jamais verrouillées mais tracées. **Alertes email** configurables (destinataire + seuil). **Historique complet** des tentatives (email, IP, résultat, date). Déblocage d'un utilisateur directement depuis la page Utilisateurs. **Rate limiting HTTP** en complément (10 échecs/15 min/IP sur les routes d'identifiants, 300 req/15 min/IP sur `/api/auth`). **Alerte email automatique** lors d'une connexion depuis un appareil inconnu (toggle activé par défaut) — l'appareil est identifié par un cookie httpOnly longue durée posé au premier login (repli : nom d'appareil + sous-réseau IP).
- 📊 **État du système** (*Admin → Système → État du système*) : uptime, mémoire, latence/taille de la base de données, dernière sauvegarde (avec bandeau rouge si la dernière sauvegarde automatique a échoué), files d'envoi, et tableau des services de fond avec leur dernier cycle (Actif / En retard / Démarré). Alimenté par `GET /api/health` (public, utilisable comme healthcheck Docker/Uptime Kuma/NPM) et `GET /api/admin/system-status`.
- 🔔 **Alertes système par e-mail** : un vérificateur de fond alerte les administrateurs quand un service de fond manque plusieurs cycles ou qu'une sauvegarde automatique échoue — un seul e-mail par lot d'incidents, rappel configurable tant que ça persiste, e-mail de rétablissement ensuite. Réglages sur la page *État du système* (destinataires — vide = tous les admins actifs —, cycles manqués, fréquence des rappels), modèle `system_alert` éditable dans *Admin → SMTP & Emails*.
