...
| 2FA WebAuthn (passkeys) | Touch ID / Face ID / Windows Hello en option (`webauthn_credentials`). S'applique aux logins mot de passe et LDAP (step-up obligatoire dès qu'une passkey est enregistrée). |
| SSO OpenID Connect | Authorization code flow avec `state`/`nonce` (Synology SSO ou tout fournisseur OIDC). **Le step-up passkey local ne s'applique pas au flux SSO** : la MFA est déléguée au fournisseur d'identité — choix assumé, détaillé dans [SECURITY_AUDIT.md](SECURITY_AUDIT.md) § S4. |
| Kerberos / SPNEGO (Windows) | Ticket de service validé **hors ligne** contre un keytab : le serveur ne contacte jamais le KDC et l'hôte n'a pas à être joint au domaine. **NTLM est refusé explicitement** (mécanisme plus faible, et handshake lié à la connexion TCP qui ne survit pas à un reverse proxy). Le keytab reste un fichier monté en lecture seule, jamais une valeur en base — il échapperait sinon aux sauvegardes. Restriction par plages CIDR : `Negotiate` n'est annoncé qu'aux réseaux déclarés. **Comme pour le SSO, le step-up passkey local ne s'applique pas** — voir [SECURITY_AUDIT.md](SECURITY_AUDIT.md) § S4. |
