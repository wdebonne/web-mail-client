...
import {
  ...
  Eye, CloudUpload, CloudDownload, RefreshCw, Users as UsersIcon, AlertCircle, Check, Link2, ChevronDown,
} from 'lucide-react';
...
function CalendarUserAutocomplete({ userId, setUserId, users }: {
  userId: string;
    ...
    <div className="relative flex-1">
      <button
        ...
        className="w-full px-2 py-1.5 text-sm border border-outlook-border rounded flex items-center justify-between gap-1
                   bg-white dark:bg-outlook-bg-dark hover:border-outlook-blue focus:outline-none focus:ring-1 focus:ring-outlook-blue"
      >
        <span className={`truncate ${selected ? '' : 'text-outlook-text-secondary'}`}>
          {selected ? (selected.display_name || selected.email) : '— Sélectionner un utilisateur —'}
        </span>
      ...
      {open && (
        <>
          ...
          <div className="absolute z-[70] mt-1 w-full min-w-[220px] bg-white dark:bg-outlook-bg-dark border border-outlook-border rounded shadow-lg">
            <div className="p-2 border-b border-outlook-border">
              <input
                ...
                className="w-full px-2 py-1 text-sm border border-outlook-border rounded
                           focus:outline-none focus:ring-1 focus:ring-outlook-blue bg-white dark:bg-outlook-bg-dark"
              />
            </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="pt-3 border-t border-outlook-border">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <CalendarUserAutocomplete userId={newUserId} setUserId={setNewUserId} users={availableUsers} />
              <select
                value={newPermission}
