...
> - Fallback legacy accepté transitoirement pour ne pas invalider les sessions existantes — il contourne la révocation d'appareils, suivi en § S2 (« Points connus, non corrigés »).
...
## 🟣 Points connus, non corrigés (suivi — juillet 2026)

Reports connus, revérifiés en juillet 2026. Ils sont listés ici pour être suivis (ou assumés) explicitement, et ne pas être re-signalés à chaque passe d'audit. Les points corrigés depuis restent listés, barrés, pour l'historique.

### S1. Dépendance `dav` vulnérable — en réalité inutilisée

**Fichier :** [server/package.json](server/package.json#L21)

`dav@^1.8.0` porte le **seul finding critical restant** de `npm audit`. La migration vers `tsdav` était envisagée, mais une vérification (juillet 2026) montre que **le paquet n'est importé nulle part** dans le projet : les synchronisations CalDAV/CardDAV ([server/src/services/caldav.ts](server/src/services/caldav.ts), [server/src/services/carddav.ts](server/src/services/carddav.ts)) reposent sur du code HTTP maison.

**Correctif :** supprimer la dépendance (`npm uninstall dav` dans `server/`) — aucune migration de code nécessaire.

### S2. Chemins d'authentification hérités (30 j) hors du périmètre de la révocation d'appareils

**Fichiers :** [server/src/middleware/auth.ts](server/src/middleware/auth.ts#L30-L67), [server/src/index.ts](server/src/index.ts#L109)

La révocation immédiate décrite en H2 ne s'applique qu'aux access tokens porteurs d'un `sid` vérifié contre `device_sessions`. Deux chemins y échappent :

- **Cookie de session express** (`connect.sid`, `maxAge` 30 j) : toujours posé à chaque login (`req.session.userId`) et honoré **en priorité** par `authMiddleware`, sans aucun lien avec `device_sessions`. Révoquer un appareil (panneau Appareils, utilisateur ou admin) n'invalide pas ce cookie.
- **JWT legacy 30 j** (signés `SESSION_SECRET`) : plus émis depuis le rollout des refresh tokens (`generateToken` n'a plus aucun appelant), mais toujours **acceptés en repli** par `authMiddleware`, sans vérification de session.

**Correctif envisagé :** lier la session express à un `sid` (ou la détruire lors de la révocation de l'appareil correspondant), puis supprimer le repli JWT legacy — les derniers tokens émis avant le rollout sont expirés depuis longtemps.

### S3. ~~Import LDAP — aucun filtre sur les groupes synchronisés~~

**Fichier :** [server/src/services/ldap.ts](server/src/services/ldap.ts#L143-L195)

La phase 2 (auto-sync) de `syncLdapGroups` créait un groupe applicatif pour **chaque** DN présent dans le `memberOf` de l'utilisateur, sans liste d'inclusion/exclusion. Sur un annuaire d'entreprise, les groupes techniques (distribution, sécurité, licences…) polluaient la liste des groupes de l'application.
